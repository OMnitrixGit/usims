 ...
 *************************************************************/
//(1) 查询表中所有数据
var ALL = () => `SELECT * FROM doctor`;
var LOGIN = (workNumber) => `SELECT * FROM doctor WHERE workNumber="${workNumber}";`;
//(2) 插入数据(全部列)
...
var REGISTER = (workNumber,password,name,gender,contact,permission) => `INSERT INTO doctor(workNumber,password,name,gender,contact,permission) VALUES("${workNumber}","${password}","${name}","${gender}","${contact}","${permission}");`;
//(4) 删除数据(根据id)
